...
@override
State<CompanyProfileCardWithUpload> createState() => _CompanyProfileCardWithUploadState();
}

class _CompanyProfileCardWithUploadState extends State<CompanyProfileCardWithUpload> {
...
final form = FormData.fromMap({'logo': await MultipartFile.fromFile(file.path, filename: filename)});
...
final msg = resp?.data != null && resp?.data['message'] != null ? resp?.data['message'].toString() : 'upload_failed'.tr();
showCustomToast(context: context, title: 'error'.tr(), description: msg ?? 'upload_failed'.tr(), icon: Icons.error, primaryColor: Colors.red);
}
} on DioError catch (e) {
debugPrint('Logo upload DioError: ${e.type} ${e.message} resp=${e.response?.statusCode} ${e.response?.data}');
final friendly = (e.response?.statusCode == 404) ? 'not_found_endpoint'.tr() : (e.message ?? 'image_upload_failed'.tr());
...
return SafeArea(
child: Wrap(children: [
ListTile(leading: const Icon(Icons.photo_library), title: Text('choose_from_gallery'.tr()), onTap: () {
Navigator.pop(ctx);
_pickImageAndUpload(ImageSource.gallery);
}),
ListTile(leading: const Icon(Icons.photo_camera), title: Text('take_photo'.tr()), onTap: () {
Navigator.pop(ctx);
...
Row(children: [
GestureDetector(
onTap: widget.allowPick ? () => _showImagePickerOptions(allowCompanyLogo: true) : null,
child: CircleAvatar(
key: ValueKey(company.logo ?? ''),
radius: 30,
backgroundColor: AppColors.primaryColor(context).withValues(alpha: 0.1),
backgroundImage: (logoUrl != null) ? NetworkImage(logoUrl) : null,
child: (logoUrl == null) ? Text(company.name?.substring(0, 1).toUpperCase() ?? '', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: AppColors.primaryColor(context))) : null,
),
),
const SizedBox(width: 16),
Expanded(child: Text(company.name ?? '', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: AppColors.primaryColor(context)))),
]),
const Divider(),
...
_infoRow('Status', company.isActive == true ? 'Active' : 'Inactive', context),
]),
),
...
const Icon(Icons.cloud_upload, size: 32, color: Colors.white),
]),
),
...
SizedBox(width: 130, child: Text(label, style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.text(context), fontSize: 13))),
