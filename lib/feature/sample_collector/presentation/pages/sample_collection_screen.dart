...
class SampleCollectionScreen extends StatefulWidget {
  ...
  @override
  State<SampleCollectionScreen> createState() => _TransactionScreenState();
}

class _TransactionScreenState extends State<SampleCollectionScreen> {
...
// default value
  ...
  void initState() {
    super.initState();
    context.read<BoothBloc>().add(LoadBooth());
    context.read<CollectorBloc>().add(LoadCollector());
    _loadInvoices();
  ...
  Future<void> _loadInvoices({
    ...
    debugPrint(
        'Loading invoices - pageNumber: $pageNumber, pageSize: $itemsPerPage');
      ...
      child: SafeArea(
        child: MultiBlocListener(
          listeners: [
            ...
            BlocListener<InvoiceUnSyncBloc, InvoiceUnSyncState>(
              listener: (context, state) async {
                if (state is InvoiceUnSyncLoading) {
                  ...
                  // for (var i in state.invoices.data ?? []) {
                  //   if (state.isCreate == false) {
                  //     syncBloc.add(SyncInvoiceAndPatient(
                  //       invoice: i.invoice!.toJson(),
                  //       patient: i.patient!.toJson(),
                  //       moneyReciptList:
                  //           (i.moneyReceipt?.map((e) => e.toJson()).toList() ??
                  //                   [])
            ...
            BlocListener<SampleCollectorBloc, SampleCollectorState>(
              listener: (context, state) async {
                if (state is SampleCollectorLoading) {
                  appLoader(context, "Loading in, please wait...");
                } else if (state is SampleCollectorLoaded) {
                  AppRoutes.pop(context);
                  _applyFiltersAndFetch();
                } else if (state is SampleCollectorError) {
                  AppRoutes.pop(context);
                  showCustomToast(
                    context: context,
                    title: 'Failed!',
