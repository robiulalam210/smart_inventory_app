...
class SupplierDataTableWidget extends StatelessWidget {
  ...
  DataRow _buildRow(int index, SupplierListModel supplier) {
    ...
    return DataRow(cells: [
      ...
      DataCell(Text(supplier.supplierNo ?? '-')),
      DataCell(Text(supplier.name ?? '-')),
      DataCell(Text(supplier.address ?? '-')),
      DataCell(Text(formatCurrency(supplier.totalPurchases))),
      DataCell(Text(formatCurrency(supplier.totalPaid))),
      DataCell(Text(formatCurrency(supplier.totalDue))),

      DataCell(
        Text(
          supplier.status ?? '-',
          style: TextStyle(
            color: supplier.status == 'Active' ? Colors.green : Colors.grey,
          ),
        ),
      ),
    ]);
